...
export default function TicketDAO() {
    ...
    this.getTicket = (id_service) => {
        ...
        return new Promise((resolve, reject)=>{
            const query = 'SELECT max(ticket_number) as ticket_number FROM tickets WHERE id_service = ? AND date = ?';
            db.all(query, [id_service, formattedDate], (err, row) => {
                ...
                if (row === undefined) {
                    ...
                    const insertQuery = 'INSERT INTO tickets (id_service, ticket_number) VALUES (?, ?)';
